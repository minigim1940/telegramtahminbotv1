...
**Sorularınız için:** @YourSupportUsername
    ...
    async def today_matches(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        response += f"**📊 Toplam:** {len(matches)} maç bulundu!\n\n"
        
        match_count = 0
        league_count = 0
        
        for league_key, league_matches in sorted_leagues:
            if match_count >= 40:  # Maksimum 40 maç göster
                break
            
            if league_count >= 15:  # Maksimum 15 lig göster
                break
                
            response += f"🏆 **{league_key}**\n"
            league_count += 1
            
            for match in league_matches[:8]:  # Her ligden max 8 maç
                if match_count >= 40:
                    break
        ...
        if match_count < len(matches):
            response += f"\n� **Not:** İlk {match_count} maç gösteriliyor.\n"
            response += f"Toplam {len(matches)} maç var.\n\n"
        
        response += "� **Tahmin almak için:**\n"
        response += "📱 /tahmin[KOD] yazın\n"
        response += "📝 Örnek: /tahmin1479574\n"
    ...
    async def specific_prediction(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
